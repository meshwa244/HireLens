...
- **OFFICIAL HACKATHON DATASET LIVE**: JD "Junior Full Stack Developer Intern" @ TechNova Solutions (real JD PDF) + all 18 official resumes (Resume_01_Aditi_Sharma … Resume_18_Pooja_Shetty) in `/app/backend/dataset/`. Official ranking: Aditi Sharma 40.31, Priya Menon 39.36, Meera Pillai 37.55 … Mohammed Faizan 0.46 — full gradient, deterministic run-twice identical.
- **JD extraction hardened**: section-aware splitter (MUST-HAVE vs GOOD-TO-HAVE, line-anchored headers) merged with LLM output → 13 required + 8 preferred requirements from the real JD. New aliases: MySQL, Jest, Mocha, REST APIs.
- **Bonus criteria done**: (1) JD bias/narrowness flags — degree-rigidity + mandatory-density heuristics + exclusionary-language detection + LLM critique w/ fallback; (2) Recruiter AI answers "Why is X ranked above Y?" for ANY named pair (name-aware deterministic fallback + full-pool LLM grounding); (3) messy-resume tolerance — 40+ header aliases, typo correction (PROJETS→projects etc.), unicode bullet/dash cleanup, verified on mangled input.
